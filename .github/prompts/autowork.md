...
a. **Read the target issue and check eligibility.** Eligible = open, unassigned or assigned to designated bot persona (`vars.AGENT_BOT_LOGIN` or `@jonah-fleet-bot`) or reclaimable stale claim, no open PR (`Closes #N`), no unclosed inward blocking dependencies (`Blocked by #N` or `Depends on #N` where `#N` is open), and not carrying `needs-human` or `needs-design`.
...
7. Call `get_me` once to learn your own GitHub account (`login`).
   - **Persona & Bot Login Matching**: In environments with designated machine user accounts or persona assignment triggers (`vars.AGENT_BOT_LOGIN` or `@jonah-fleet-bot`), ensure `get_me` identity matching respects both your authenticated account and the configured bot persona. An issue or PR assigned to the bot persona is treated as self-assigned for claim and ownership checks.
