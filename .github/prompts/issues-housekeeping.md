...
- [ ] Stalled routine run audit and stale failure sweep completed: routine log issues in `status:running` older than 6 hours marked as `status:failure`; `status:failure` routine log issues referencing an already closed/merged target closed immediately; untargeted `status:failure` runner crash issues older than 12 hours closed with resolution summary
...
1. **Stalled routine run and stale failure sweep**:
   ...
   - **Stale / untargeted failures**: Check open issues with labels `routine-log` and `status:failure`. Reconcile and close them as follows:
     - If the routine log references a target issue or PR that has already been closed or merged: close the routine-log issue **immediately** (0-hour delay) with comment "Closed: target issue/PR has been resolved.\n\n_Generated by [Antigravity](${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/actions/runs/${GITHUB_RUN_ID:-})_" and reason completed.
     - If the routine log has no target issue or PR in its title or body (an untargeted runner crash), or if its referenced target is a generic routine name (`autowork`, `peer-review`), circular self-reference (`Issue #<self>`), or another routine-log issue (`[routine] run ...`): treat it as an untargeted failure. If older than 12 hours, close the routine-log issue with comment "Closed: untargeted routine failure superseded.\n\n_Generated by [Antigravity](${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/actions/runs/${GITHUB_RUN_ID:-})_" and reason completed.
