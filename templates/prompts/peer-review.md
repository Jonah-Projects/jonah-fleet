  ...
  - **Post-Merge Companion Measurement Issue Filing**:
    ...
    - If a companion measurement issue is outlined (e.g. `Measure: <Feature Name>`) or if the feature introduced Tier 1 / Tier 2 telemetry contracts:
      1. Check if an open tracker already exists:
         ```bash
         gh issue list --label "measurement" --search "Measure: <Feature Name> in:title" --state open
         ```
      2. If no open tracker exists, synthesize and file the companion measurement issue with the `measurement` label so `analytics-review` can track its adoption (using a quoted heredoc to avoid shell expansion and markdown backtick interpretation issues):
         ```bash
         gh issue create \
           --title "Measure: <Feature Name>" \
           --label "measurement" \
           --body-file - <<'EOF'
Companion measurement issue for PR #<PR_NUMBER> (closes #<ISSUE_NUMBER>).
