...
async function main() {
  ...
  let routine = process.env.ROUTINE;
  if (!routine) {
    for (let i = 0; i < commandArgs.length; i++) {
      if (commandArgs[i] === '-p' || commandArgs[i] === '--prompt') {
        const promptText = commandArgs[i + 1] || '';
        if (/analytics[-_ ]review/i.test(promptText)) {
          routine = 'analytics-review';
        } else if (/peer[-_ ]review/i.test(promptText)) {
          routine = 'peer-review';
        } else if (/design[-_ ]review/i.test(promptText)) {
          routine = 'design-review';
        } else if (/issues[-_ ]housekeeping/i.test(promptText)) {
          routine = 'issues-housekeeping';
        } else if (/dependency[-_ ]update/i.test(promptText)) {
          routine = 'dependency-update-security-check';
        } else if (/product[-_ ]planning/i.test(promptText)) {
          routine = 'product-planning';
        } else if (/optimizer/i.test(promptText)) {
          routine = 'optimizer';
        }
        break;
      }
